function $(selector) {
	...
	self.kowlider = function (entries = '') {
		options = {
			...
			fitParent: entries.fitParent ? entries.fitParent : false,
			loop: entries.loop === false ? entries.loop : true,
			customize: {
				width: false,
				height: false,
			},
			...
			pauseOnHover: entries.pauseOnHover ? entries.pauseOnHover : false,
			effect: entries.effect ? entries.effect : false,
		};

		// If default font file exist :
		checkIfFileExists('../font/fontello.svg', function(exists) {
			if (exists) {
				source.elements.slideControlNav.arrow = ['&#xe800;', '&#xe801;']; //fontello
			} else {
				source.elements.slideControlNav.arrow = ['&lsaquo;', '&rsaquo;']; //html
			}
			init(options);
		});
	};
...
function variable() {
	...
	let slideControlNavArrowLeft = source.elements.slideControlNav.arrow[0];
	let slideControlNavArrowRight = source.elements.slideControlNav.arrow[1];
...
function autoplaySlide(options) {
	...
	let tps = options.autospeed;
	if(tps>=30||tps<1)tps=5000;
	if(tps<30)tps=tps*1000;
...
function nextSlide(options) {
    ...
    if (currentSlide === lastSlide && options.loop === false) {
        return;
    }
...
function prevSlide(options) {
    ...
    if (currentSlide === firstSlide && options.loop === false) {
        return;
    }
...
function displaySlide(options) {
	...
	width: ${options.fitParent ? '100%' : `${slider.clientWidth}px`};
	height: ${options.fitParent ? '100%' : `${slider.clientHeight}px`};
	background-image : url("${currentImage}");
	background-size: ${options.fitParent ? '100% 100%' : `${slider.clientWidth}px ${slider.clientHeight}px`};
	`;
...
function stripAnimation(current, options) {
	...
	} else if (options.effect === 'sliceUpDown' || options.effect === 'sliceRotateX' || options.effect === 'sliceMixUpDown' || options.effect === 'sliceRightBottomToUp' || options.effect === 'sliceRightTopToDown' || options.effect === 'sliceLeftBottomToUp' || options.effect === 'sliceLeftTopToDown' || options.effect === 'sliceLeftToRight' || options.effect === 'sliceRightToLeft' || options.effect === 'sliceTopToBottom') {
		for (let i = 0; i < slideDisplay.children.length; i++) {
			const strip = slideDisplay.children[i];
			strip.style.opacity = '1';
			strip.style.backgroundImage = `url("${currentImage}")`;
...
function stripStyling(properties) {
	...
	if (options.effect === 'sliceUpDown' || options.effect === 'sliceRotateX' || options.effect === 'sliceMixUpDown' || options.effect === 'sliceRightBottomToUp' || options.effect === 'sliceRightTopToDown' || options.effect === 'sliceLeftBottomToUp' || options.effect === 'sliceLeftTopToDown' || options.effect === 'sliceLeftToRight' || options.effect === 'sliceRightToLeft') {
		strip.style.width = `${stripSize}px`;
		strip.style.height = `${totalHeight}px`;
		strip.style.backgroundPositionX = `-${stripSize * i}px`;
		strip.style.left = `${stripSize * i}px`;
...
function createCaption(txt, options) {
	...
	if (txt.getAttribute('data-caption') === 'one') {
	...
	if (txt.getAttribute('data-caption') === 'two') {
	...
	if (txt.getAttribute('data-caption') === 'three') {
...
function getSlideData(options) {
	...
	const dts = slider.querySelectorAll('[data-caption]');
	...
	imgs.forEach((image, i) => {
		...
		// add options : data-href attribute to click 
		const attributesToCopy = ['data-href', 'data-option', 'target'];
		attributesToCopy.forEach(attr => {
			if (image.hasAttribute(attr)) {
				slide.setAttribute(attr, image.getAttribute(attr));
			}
		});
		...
		if (options.caption) {
			for (let a = 0; a < dts.length; a++) {
				const txt = dts[a];
				if (image.getAttribute('data-txt') === txt.getAttribute('data-txt')) {
					...
					if ([`${one}`, `${two}`, `${three}`].includes(txt.getAttribute('data-caption'))) {
	...
	Array.from(allSlide.children).forEach(slide => {
		slide.addEventListener('click', (event) => {
			const link = slide.getAttribute('data-href');
			const option = slide.getAttribute('data-option');
			const target = slide.getAttribute('target');
			if (link) {
				if(option==="_blank" || option==="blank" || target==="_blank") {
